...
public class Day5 {
    ...
    private static void part1(String input) {
        ...
        Lookup root = new Lookup();
        {
            ...
            while (it.hasNext()) {
                ...
                if ("seeds".equals(header)) {
                    seeds = Stream.of(line.substring(line.indexOf(':') + 2).split(" ")).map(Long::parseLong).toList();
                    if (!it.next().isBlank())
                    ...
                    if (current == null)
                        ...
                        current = current.newLookup();
                    }
                    readMapLines(it, current);
                }
            }
        }
        {
            ...
            for (Long seed : seeds) {
                for (Long x : root.find(new Range(seed, seed + 1))) {
                    min = Math.min(min, x);
                }
            }
        ...
        long min = Long.MAX_VALUE;
        for (List<Long> longs : Lists.partition(seeds, 2)) {
            long start = longs.get(0);
            long end = longs.get(1) + start;
            for (Long x : root.find(new Range(start, end))) {
                min = Math.min(min, x);
            }
    ...
    private static void readMapLines(Iterator<String> it, Lookup lookup) {
        while (it.hasNext()) {
            String line = it.next();
            if (line.isBlank())
                break;
            ...
            lookup.add(new MapLine(new Range(start, start + length), dest));
        }
    }
    ...
    public record MapLine(Range range, long dest) {
        public long length() {
    ...
    public static class Lookup {
        private final List<MapLine> maplines = new ArrayList<>();
        private Lookup next;
        ...
        public List<Long> find(Range inputRange) {
            List<Long> results = new ArrayList<>();

            List<Range> unmappedRanges = new ArrayList<>();
            unmappedRanges.add(inputRange);


            for (MapLine mapline : maplines) {
                Range mapRange = mapline.range();


                List<Range> copy = new ArrayList<>(unmappedRanges);
                unmappedRanges.clear();
                for (Range range : copy) {

                    if (range.intersects(mapRange)) {

                        if (range.start() < mapRange.start()) {
                            unmappedRanges.add(new Range(range.start(), mapRange.start()));
                        }
                        if (range.end() > mapRange.end()) {
                            unmappedRanges.add(new Range(mapRange.end(), range.end()));
                        }
                        Range overlap = new Range(Math.max(range.start(), mapRange.start()), Math.min(range.end(), mapRange.end()));

                        long offset = overlap.start() - mapRange.start();

                        results.addAll(findNext(new Range(mapline.dest() + offset, mapline.dest() + overlap.length() + offset)));
                    } else
            ...
            for (Range r : unmappedRanges) {
                results.addAll(findNext(r));
            }

            return results;
        }

        private List<Long> findNext(Range range) {
            if (next == null)
                return List.of(range.start());
            return next.find(range);
        }
