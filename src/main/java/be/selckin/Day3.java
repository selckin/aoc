        ...
        for (int y = 0; y < grid.size(); y++) {
            ...
            for (int x = 0; x < line.length(); x++) {
                ...
                if (!Character.isDigit(cur) && cur != '.') {
                    ...
                    find(partNumbers, grid, x, y - 1);
                    find(partNumbers, grid, x, y);
                    find(partNumbers, grid, x, y + 1);
                    if (cur == '*' && partNumbers.size() == 2) {
                        numbers.add(partNumbers.get(0) * partNumbers.get(1));
                    }
                }
            }
        }

        System.out.println(numbers.stream().mapToInt(Long::intValue).sum());
    }

    private static void find(List<Long> numbers, List<String> grid, int x, int y) {
        if (y < 0 || y >= grid.size())
            return;


        String line = grid.get(y);
        if (!findNumber(numbers, line, x)) {
            findNumber(numbers, line, x - 1);
            findNumber(numbers, line, x + 1);
        }
    }

    private static boolean findNumber(List<Long> numbers, String line, int x) {
        if (!Character.isDigit(line.charAt(x)))
            return false;
        int start = -1;
        for (int i = x; i >= 0 && Character.isDigit(line.charAt(i)); i--) {
            start = i;
        }

        int end = -1;
        for (int i = x; i < line.length() && Character.isDigit(line.charAt(i)); i++) {
            end = i;
        }
