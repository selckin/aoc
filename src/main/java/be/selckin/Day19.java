...
public class Day19 {
    ...
    public static void main(String[] args) throws IOException {
        part1(PART1_EXAMPLE, 19114);
        part1(PART1_INPUT, 376008);
        System.out.println("==============");
        part2(PART1_EXAMPLE, 167409079868000L);
        part2(PART1_INPUT, 124078207789312L);
    }

    public record Ranges(Range x, Range m, Range a, Range s) {
        public Ranges x(Range x) {
            return new Ranges(x, m, a, s);
        }

        public Ranges m(Range m) {
            return new Ranges(x, m, a, s);
        }

        public Ranges a(Range a) {
            return new Ranges(x, m, a, s);
        }

        public Ranges s(Range s) {
            return new Ranges(x, m, a, s);
        }
    ...
    public record Range(int start, int end) {
        ...
        // x > value
        private Range update(String operator, int value) {
            return switch (operator) {
                case "<" -> new Range(start, Math.min(end, value - 1));
                case ">" -> new Range(Math.max(start, value + 1), end);
                default -> throw new IllegalStateException();
            };
        }

        private Range inverse(String operator, int value) {
            return switch (operator) {
                case ">" -> new Range(start, Math.min(end, value));
                case "<" -> new Range(Math.max(start, value), end);
                default -> throw new IllegalStateException();
            };
        }
    ...
    private static void part2(String input, long expected) {
        ...
        List<Ranges> ranges = new Foo(data.workflows).test();
        ...
        long result = ranges.stream().mapToLong(Ranges::multiply).sum();
    ...
    public static class Foo {
        ...
        private final List<Ranges> accepted = new ArrayList<>();

        public Foo(Map<String, Workflow> workflows) {
            this.workflows = workflows;
        }

        private List<Ranges> test() {
            Range range = new Range(1, 4000);
            test(
                    new Ranges(range, range, range, range),
                    workflows.get("in")
            );
            return accepted;
        }

        private void test(Ranges ranges, Workflow workflow) {
            if (!ranges.isValid())
                return;

            for (Rule rule : workflow.rules()) {
                if ("A".equals(rule.target)) {
                    accepted.add(apply(ranges, rule));
                } else if (!"R".equals(rule.target)) {
                    test(apply(ranges, rule), workflows.get(rule.target));
                }

                ranges = inverse(ranges, rule);
            }
        }

        private static Ranges apply(Ranges ranges, Rule rule) {
            if (rule.xmas == null)
                return ranges;
            return switch (rule.xmas) {
                case "x" -> ranges.x(ranges.x.update(rule.operator, rule.value));
                case "m" -> ranges.m(ranges.m.update(rule.operator, rule.value));
                case "a" -> ranges.a(ranges.a.update(rule.operator, rule.value));
                case "s" -> ranges.s(ranges.s.update(rule.operator, rule.value));
                default -> throw new IllegalStateException();
            };
        }

        private static Ranges inverse(Ranges ranges, Rule rule) {
            if (rule.xmas == null)
                return ranges;

            return switch (rule.xmas) {
                case "x" -> ranges.x(ranges.x.inverse(rule.operator, rule.value));
                case "m" -> ranges.m(ranges.m.inverse(rule.operator, rule.value));
                case "a" -> ranges.a(ranges.a.inverse(rule.operator, rule.value));
                case "s" -> ranges.s(ranges.s.inverse(rule.operator, rule.value));
                default -> throw new IllegalStateException();
            };
        }
    }
    ...
    private static void part1(String input, long expected) {
        ...
        for (Part part : data.parts) {
            if (test(data.workflows, part, data.workflows.get("in")))
                accepted.add(part);
        }

        int result = accepted.stream().mapToInt(part -> part.x + part.m + part.a + part.s).sum();

        if (expected != result)
            throw new RuntimeException("" + result);
    }

    private static boolean test(Map<String, Workflow> workflows, Part part, Workflow workflow) {
        ...
        if ("A".equals(result)) {
            ...
            return test(workflows, part, workflows.get(result));
        }
    }
    ...
    public record Rule(String xmas, String operator, int value, String target) {
    ...
    public record Data(List<Part> parts, Map<String, Workflow> workflows) {


    }

    public static Pattern PART = Pattern.compile("\\{x=(\\d+),m=(\\d+),a=(\\d+),s=(\\d+)\\}");
    ...
    private static Data parse(String input) {
        ...
        input.lines().forEach(line -> {
            ...
            if (line.startsWith("{")) {
                ...
                parts.add(new Part(Integer.parseInt(matcher.group(1)), Integer.parseInt(matcher.group(2)), Integer.parseInt(matcher.group(3)), Integer.parseInt(matcher.group(4))));
            } else {
                int start = line.indexOf('{');
                String name = line.substring(0, start);
                List<Rule> rules = new ArrayList<>();
                for (String rule : line.substring(start + 1, line.indexOf('}')).split(",")) {
                    int semi = rule.indexOf(':');
                    if (semi == -1) {
                        rules.add(new Rule(null, null, -1, rule));
                    } else {
                        String target = rule.substring(semi + 1);
                        String[] split = rule.substring(0, semi).splitWithDelimiters("[<>]", 0);
                        if (split.length != 3)
                            throw new RuntimeException();

                        rules.add(new Rule(split[0], split[1], Integer.parseInt(split[2]), target));
                    }
                }
        ...
        return new Data(parts, workflows);
    }
