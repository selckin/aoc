...
public class Day2 {
    ...
    private static void part1(String input) throws IOException {
        ...
        for (Game game : games) {
            ...
            for (Pick pick : game.picks()) {
                maxRed = Math.max(maxRed, pick.red());
                maxGreen = Math.max(maxGreen, pick.green());
                maxBlue = Math.max(maxBlue, pick.blue());
            }
    ...
    private static List<Game> parseGames(String input) {
        var games = input.lines().map(line -> {
            int header = line.indexOf(':');
            int id = Integer.parseInt(line.substring("Game".length(), header).trim());

            List<Pick> picks = Pattern.compile(";").splitAsStream(line.substring(header + 1)).map(pickLine -> {
                int red = 0;
                int green = 0;
                int blue = 0;

                for (String pick : pickLine.split(",")) {
                    pick = pick.trim();
                    int idx = pick.indexOf(" ");
                    String color = pick.substring(idx).trim();
                    int value = Integer.parseInt(pick.substring(0, idx));
                    switch (color) {
                        case "red" -> red = value;
            ...
            return new Game(id, picks);
        }).toList();
        return games;
    }

    private static boolean isPossible(int red, int green, int blue, Game game) {
        for (Pick pick : game.picks()) {
            if (pick.red() > red || pick.green() > green || pick.blue() > blue)
                return false;
        }
